...
func applyPatch(target *Task, patch *taskPatch) {
	...
	if patch.TaskName != nil { target.TaskName = *patch.TaskName }
	if patch.SiteAdapter != nil { target.SiteAdapter = *patch.SiteAdapter }
	if patch.TargetBoardURL != nil { target.TargetBoardURL = *patch.TargetBoardURL }
	if patch.SaveRootDirectory != nil { target.SaveRootDirectory = *patch.SaveRootDirectory }
	if patch.DirectoryFormat != nil { target.DirectoryFormat = *patch.DirectoryFormat }
	if patch.FilenameFormat != nil { target.FilenameFormat = *patch.FilenameFormat }
	if patch.SearchKeyword != nil { target.SearchKeyword = *patch.SearchKeyword }
	if patch.ExcludeKeywords != nil { target.ExcludeKeywords = *patch.ExcludeKeywords }
	if patch.MinimumMediaCount != nil { target.MinimumMediaCount = *patch.MinimumMediaCount }
	if patch.WatchIntervalMillis != nil { target.WatchIntervalMillis = *patch.WatchIntervalMillis }
	if patch.MaxConcurrentDownloads != nil { target.MaxConcurrentDownloads = *patch.MaxConcurrentDownloads }
	if patch.CatalogTitleLength != nil { target.CatalogTitleLength = *patch.CatalogTitleLength }
	if patch.PostContentFilters != nil { target.PostContentFilters = patch.PostContentFilters }
	if patch.HistoryFilePath != nil { target.HistoryFilePath = *patch.HistoryFilePath }
	if patch.VerificationHistoryPath != nil { target.VerificationHistoryPath = *patch.VerificationHistoryPath }
	if patch.MetadataIndexPath != nil { target.MetadataIndexPath = *patch.MetadataIndexPath }
	if patch.LogFilePath != nil { target.LogFilePath = *patch.LogFilePath }
	if patch.RetryCount != nil { target.RetryCount = *patch.RetryCount }
	if patch.RetryWaitMillis != nil { target.RetryWaitMillis = *patch.RetryWaitMillis }
	if patch.RequestTimeoutMillis != nil { target.RequestTimeoutMillis = *patch.RequestTimeoutMillis }
	if patch.RequestIntervalMillis != nil { target.RequestIntervalMillis = *patch.RequestIntervalMillis }
	if patch.NotifyOnComplete != nil { target.NotifyOnComplete = *patch.NotifyOnComplete }
	if patch.NotifyOnError != nil { target.NotifyOnError = *patch.NotifyOnError }
	if patch.EnableHistorySkip != nil { target.EnableHistorySkip = *patch.EnableHistorySkip }
	if patch.EnableResumeSupport != nil { target.EnableResumeSupport = *patch.EnableResumeSupport }
	if patch.EnableLogFile != nil { target.EnableLogFile = *patch.EnableLogFile }
	if patch.LogLevel != nil { target.LogLevel = *patch.LogLevel }
	if patch.EnableMetadataIndex != nil { target.EnableMetadataIndex = *patch.EnableMetadataIndex }
	if patch.MetadataIndexFormat != nil { target.MetadataIndexFormat = *patch.MetadataIndexFormat }
	if patch.FutabaCatalogSettings != nil { target.FutabaCatalogSettings = patch.FutabaCatalogSettings }
}
